...
export default function MiGrupoPublicadoresPage() {
  ...
  const currentGroupName = useMemo(() => {
      if (currentGroupIdForManagement) {
          const sgOfGroup = MOCK_ALL_USERS_DATA.find(p => p.assignedGroupId === currentGroupIdForManagement && p.role === USER_ROLES.SG);
          if (sgOfGroup) return `Grupo de ${sgOfGroup.name.split(' ')[0]}`; 
          const groupInfo = MOCK_ALL_USERS_DATA.find(p => p.assignedGroupId === currentGroupIdForManagement);
